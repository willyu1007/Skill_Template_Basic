...
- The init kit is intentionally **bootstrap-only**. If you remove `init/`, archive Stage A + blueprint to `docs/project/overview/` (or override with `--archive-dir`) if you want to keep them as long-term assets, and keep:
  - `docs/project/overview/` artifacts (when archived), and
...
- Stage C mutates the top-level keys:
  ...
  - optional `includeSkills` (from blueprint `skills.includeSkillNames`)
...
### 4.1 Optional: prune `agent-builder` (heavy)

**Option A: During Stage C apply**

If the user does not need the `agent-builder` workflow, run Stage C with:
...
If the decision to remove `agent-builder` is made after initialization, use:
...
- Use `--archive`, `--archive-docs`, or `--archive-blueprint` to copy init inputs into `docs/project/overview/` before cleanup (or override with `--archive-dir`).
