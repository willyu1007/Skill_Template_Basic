...
4. Optional (recommended for accuracy when hub exists): run
   ...
   - If lint reports errors, include a remediation command (`node .ai/scripts/projectctl.mjs sync --apply --project <project>`) in the output but do not execute the write.
...
## Boundaries
- MUST NOT modify files (no `projectctl sync --apply`, no editing `dev-docs/**` / `.ai/project/**`)
